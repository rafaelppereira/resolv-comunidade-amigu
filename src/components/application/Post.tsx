...
export function Post({ author, publishedAt, content }: PostProps) {
  ...
  return (
    <div className="w-full bg-white border-2 border-zinc-300 py-10 px-10 rounded-lg mb-8">
      <header className="text-white flex justify-between items-center pb-6">
        <div className="flex">
          ...
          <div className="pl-4">
            ...
            <p className="text-[#8D8D99] text-sm">{author.role}</p>
          </div>
        </div>

        <time className="text-[#8D8D99] text-sm">{publishedDateFormatted}</time>
      </header>
      ...
      <div className="text-white">
        {content.map((line) => {
          if (line.type === 'paragraph') {
            return (
              <p key={line.content} className="pb-3">
                {line.content}
              </p>
            )
          } else if (line.type === 'link') {
            return (
              <p key={line.content} className="pt-3">
      ...
      <footer className="border-t-[1px] border-solid border-[#323238]  mt-5 pt-5 px-6 w-full"></footer>
